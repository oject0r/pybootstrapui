...
class ImageObject(HTMLElement):
	...
	def __init__(self, base64_data: bytes | str, *,
...
class FileImage(ImageObject):
	...
	def __init__(self, file_path: str | bytes | PathLike[str] | PathLike[bytes], *,
		...
		super().__init__(content, width=width, height=height, alt=alt, classes=classes, unique_id=unique_id)


class Base64Image(ImageObject):
	"""
	Represents an image directly from base64-encoded data.
	"""
	...
	def __init__(self, base64_data: bytes | str, *,
		...
		Parameters:
			...
			- `unique_id` (str | None): Unique identifier for the image element.
		"""
		super().__init__(base64_data, width=width, height=height, alt=alt, classes=classes, unique_id=unique_id)
...
class RawImage(ImageObject):
	...
	def __init__(self, image_content: bytes, *,
				 width: str = 'auto', height: str = 'auto',
				 alt: str | None = None,
				 classes: list[str] | None = None, unique_id: str | None = None):
		"""
...
async def image_from_file_async(file_path: str | bytes | PathLike[str] | PathLike[bytes], *,
				 width: str = 'auto', height: str = 'auto',
				 alt: str | None = None,
				 classes: list[str] | None = None, unique_id: str | None = None) -> ImageObject:
	"""
	...
	return ImageObject(content, width=width, height=height, alt=alt, classes=classes, unique_id=unique_id)
