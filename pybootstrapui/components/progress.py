...
class ProgressBar(HTMLElement):
	...
	def __init__(self, value: int, label: str | None = None, style: str = "primary", striped: bool = False, animated: bool = False, classes: list[str] | None = None, unique_id: str | None = None):
		"""
	...
	def change_value(self, value: int, new_label: str = ''):
		...
		add_task(f'{self.id}', 'updateProgressBar', newValue = self.value, newText = self.label)
