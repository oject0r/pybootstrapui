"""
Code is on analysis / Code is currently not working.
"""
...
class Tooltip(HTMLElement):
	...
	def __init__(self, target: HTMLElement | str, content: str, placement: str = "top", classes: list[str] | None = None, unique_id: str | None = None):
		"""
	...
	def attach(self):
		...
		"""
		target_id = self.target.id if isinstance(self.target, HTMLElement) else self.target
		add_task(target_id, "addTooltip", content=self.content, placement=self.placement)
