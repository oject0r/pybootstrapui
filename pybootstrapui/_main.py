...
class Page:
	...
	def __init__(self, template_filename: os.PathLike | str | bytes,
				 content: list[HTMLElement] = None, page_title: str | None = None,
	...
	async def reload(self):
		...
		for element in self.content:
			if element.special_id in self.inputs_saved_states and isinstance(element, InputObject):
	...
	def compile(self):
		...
		self.javascript += '\n' + rjsmin.jsmin_for_posers(websocket_javascript.replace('!PYBSUI.INSERTHOST', f'http://{constants.HOST}:{constants.AJAX_PORT}'))
	...
	def run_in_desktop(self, nwjs_path: os.PathLike[str] | os.PathLike[bytes] | str | bytes, *,
					   ...
					   title: str = "NW.js App", width: int = 800, height: int = 600, resizable: bool = True,
					   server_bind: str = '127.0.0.1', server_port: int = 0):
		"""
